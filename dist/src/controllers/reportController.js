...
exports.generateOilConsumptionReport = exports.generateWeeklyPetrolReport = exports.generateWeeklyDieselReport = exports.checkFlightCount = exports.generateStockCardReport = exports.exportDailyIssueReport = exports.getDailyIssueReport = void 0;
...
const getDailyIssueReport = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    `;
        ...
        const [totalResult] = yield connection.execute(countQuery, countParams);
        const [issues] = yield connection.execute(dataQuery, dataParams);
        const formattedIssues = issues.map(issue => (Object.assign(Object.assign({}, issue), { issued_by: JSON.parse(issue.issued_by) })));
        res.status(200).json({
        ...
        (0, logger_1.logEvents)(`Error in getDailyIssueReport: ${errorMessage}`, "reportLog.log");
        res.status(500).json({
...
exports.getDailyIssueReport = getDailyIssueReport;
const exportDailyIssueReport = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    `;
        ...
        const [issues] = yield connection.execute(query, queryParams);
        const formattedIssues = issues.map(issue => (Object.assign(Object.assign({}, issue), { issued_by: JSON.parse(issue.issued_by) })));
        res.status(200).json({
        ...
        const errorMessage = error instanceof Error ? error.message : 'Unknown error occurred';
        (0, logger_1.logEvents)(`Error in exportDailyIssueReport: ${errorMessage}`, "reportLog.log");
        res.status(500).json({
            error: 'Internal Server Error',
            message: errorMessage
        });
...
exports.exportDailyIssueReport = exportDailyIssueReport;
const generateStockCardReport = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    var _a, _b, _c, _d;
    const { fromDate, toDate, naccodes, generateByIssueDate } = req.body;
    const connection = yield db_1.default.getConnection();
    try {
        let targetNaccodes = [];
        if (generateByIssueDate) {
            ...
            const [uniqueNaccodes] = yield connection.execute(`
      ...
      `, [fromDate, toDate, "APPROVED"]);
        ...
        const [verifyResults] = yield connection.execute(`
      SELECT nac_code, LENGTH(nac_code) as code_length, HEX(nac_code) as code_hex
      FROM stock_details 
      WHERE nac_code LIKE ?
    `, [searchPattern]);
        const [stockDetails] = yield connection.execute(`
      SELECT 
        s.nac_code,
        s.item_name,
    ...
    `, [searchPattern]);
        ...
        for (const stock of stockDetails) {
            stock.equipment_number = (0, utils_1.normalizeEquipmentNumbers)(stock.equipment_number);
            const { primary, secondary } = (0, utils_1.processPartNumbers)(stock.part_number);
            stock.primary_part_number = primary;
            stock.secondary_part_numbers = secondary;
            stock.item_name = (0, utils_1.processItemName)(stock.item_name);
            ...
            if (fromDate && !generateByIssueDate) {
                ...
                const [preDateReceives] = yield connection.execute(`
          SELECT 
        ...
        `, [stock.nac_code, fromDate]);
                const [preDateIssues] = yield connection.execute(`
          SELECT 
        ...
        `, [stock.nac_code, fromDate]);
                const totalReceiveQty = Number((_a = preDateReceives[0]) === null || _a === void 0 ? void 0 : _a.total_quantity) || 0;
                const totalReceiveAmt = Number((_b = preDateReceives[0]) === null || _b === void 0 ? void 0 : _b.total_amount) || 0;
                const totalIssueQty = Number((_c = preDateIssues[0]) === null || _c === void 0 ? void 0 : _c.total_quantity) || 0;
                const totalIssueAmt = Number((_d = preDateIssues[0]) === null || _d === void 0 ? void 0 : _d.total_amount) || 0;
            ...
            const [issueRecords] = yield connection.execute(`
        SELECT 
      ...
      `, [stock.nac_code, ...(!generateByIssueDate && fromDate && toDate ? [fromDate, toDate] : [])]);
            ...
            if (stock.nac_code === 'GT 00000') {
                // Special handling for GT 00000: get from transaction_details
                const [gtReceiveRecords] = yield connection.execute(`
          SELECT 
        ...
        `, !generateByIssueDate && fromDate && toDate ? [fromDate, toDate] : []);
                ...
                const [normalReceiveRecords] = yield connection.execute(`
          SELECT 
        ...
        `, [stock.nac_code, ...(!generateByIssueDate && fromDate && toDate ? [fromDate, toDate] : [])]);
            ...
            if (stock.equipment_number.toLowerCase().includes('consumable')) {
                ...
                movements.filter(m => m.type === 'issue').forEach(movement => {
                    ...
                    if (dateMap.has(dateKey)) {
                        ...
                        dateMap.set(dateKey, Object.assign({}, movement));
                    }
                });
            ...
            stock.openingBalanceDate = openingBalanceDate;
        }
        const templatePath = path_1.default.join(__dirname, '../../public/templates/template_file.xlsx');
        const excelBuffer = yield excelService_1.ExcelService.generateStockCardExcel(stockDetails, templatePath);
        ...
        const errorMessage = error instanceof Error ? error.message : 'Unknown error occurred';
        (0, logger_1.logEvents)(`Error in generateStockCardReport: ${errorMessage}`, "reportLog.log");
        res.status(500).json({
            error: 'Internal Server Error',
            message: errorMessage
        });
...
exports.generateStockCardReport = generateStockCardReport;
const checkFlightCount = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    try {
       ...
       AND f.number_of_flights IS NOT NULL`, [start_date, end_date]);
        ...
        const errorMessage = error instanceof Error ? error.message : 'Unknown error occurred';
        (0, logger_1.logEvents)(`Error checking flight count: ${errorMessage}`, "fuelLog.log");
        res.status(500).json({
            error: 'Internal Server Error',
...
exports.checkFlightCount = checkFlightCount;
const generateWeeklyDieselReport = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    var _a;
    ...
    try {
        ...
        const [configResult] = yield connection.query('SELECT config_value FROM app_config WHERE config_name = ? AND config_type = "fuel"', ['valid_equipment_list_diesel']);
        ...
        if (flightCount !== undefined && flightCount !== null) {
            ...
            yield connection.query(`UPDATE fuel_records f
         JOIN issue_details i ON f.issue_fk = i.id
        ...
        const [fuelRecords] = yield connection.query(`SELECT 
      ...
      ORDER BY date, i.issued_for`, [start_date, end_date]);
        ...
        fuelRecords.forEach(record => {
            ...
            if (!dailyData)
                return; // Skip if no daily data found
            ...
            if (!equipmentData)
                return; // Skip if no equipment data found
            ...
            if (!equipmentTotal)
                return; // Skip if no equipment total found
        ...
        const outputPath = path_1.default.join(tempDir, filename);
        try {
            ...
            const [weekData] = yield connection.query(`SELECT 
          MAX(f.week_number) as week_number,
         ...
         AND i.issue_date BETWEEN ? AND ?
         AND i.approval_status = 'APPROVED'
         GROUP BY f.week_number, f.number_of_flights`, [start_date, end_date]);
            const currentWeek = ((_a = weekData[0]) === null || _a === void 0 ? void 0 : _a.week_number) || 0;
            const previousWeek = currentWeek - 1;
            // Get previous week's data
            const [previousWeekData] = yield connection.query(`SELECT 
          MAX(f.week_number) as week_number,
          MAX(f.number_of_flights) as number_of_flights,
          SUM(i.issue_quantity) as total_quantity,
          SUM(i.issue_quantity * f.fuel_price) as total_cost
         ...
         AND f.week_number = ?
         AND i.approval_status = 'APPROVED'
         GROUP BY f.week_number, f.number_of_flights`, [previousWeek]);
            ...
            if (currentWeekData.total_quantity !== prevWeekData.total_quantity) {
                sheet.cell('I161').value(currentWeekData.total_quantity > prevWeekData.total_quantity
            ...
            if (currentWeekData.number_of_flights !== prevWeekData.number_of_flights) {
                sheet.cell('I167').value(currentWeekData.number_of_flights > prevWeekData.number_of_flights
            ...
            if (currentWeekData.total_cost !== prevWeekData.total_cost) {
                sheet.cell('I172').value(currentWeekData.total_cost > prevWeekData.total_cost
            ...
            yield workbook.toFileAsync(outputPath);
        }
        catch (error) {
        ...
        res.download(outputPath, filename, (err) => {
            if (err) {
                (0, logger_1.logEvents)(`Error sending file: ${err.message}`, "fuelLog.log");
            }
            // Clean up the temporary file
            fs_1.default.unlink(outputPath, (unlinkErr) => {
                if (unlinkErr) {
                    (0, logger_1.logEvents)(`Error deleting temporary file: ${unlinkErr.message}`, "fuelLog.log");
                }
            });
        });
    }
        ...
        const errorMessage = error instanceof Error ? error.message : 'Unknown error occurred';
        (0, logger_1.logEvents)(`Error generating weekly diesel report: ${errorMessage}`, "fuelLog.log");
        res.status(500).json({
            error: 'Internal Server Error',
...
exports.generateWeeklyDieselReport = generateWeeklyDieselReport;
const generateWeeklyPetrolReport = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    var _a;
    const { start_date, end_date } = req.query;
    const connection = yield db_1.default.getConnection();
    try {
        yield connection.beginTransaction();
        // Fetch valid equipment list from app_config
        const [configResult] = yield connection.query('SELECT config_value FROM app_config WHERE config_name = ? AND config_type = "fuel"', ['valid_equipment_list_petrol']);
        if (configResult.length === 0) {
            throw new Error('Valid equipment list configuration not found');
        }
        // Parse the comma-separated equipment list
      ...
      ORDER BY date, i.issued_for`, [start_date, end_date]);
       ...
       AND i.issue_date BETWEEN ? AND ?
       AND i.approval_status = 'APPROVED'
       GROUP BY f.week_number`, [start_date, end_date]);
        const currentWeek = ((_a = weekData[0]) === null || _a === void 0 ? void 0 : _a.week_number) || 0;
        // Continue with Excel generation...
        const templatePath = path_1.default.join(__dirname, '../../public/templates/template_file.xlsx');
        // Create temp directory if it doesn't exist
        ...
        yield connection.rollback();
        const errorMessage = error instanceof Error ? error.message : 'Unknown error occurred';
        (0, logger_1.logEvents)(`Error generating weekly petrol report: ${errorMessage}`, "fuelLog.log");
        res.status(500).json({
            error: 'Internal Server Error',
            message: error instanceof Error ? error.message : 'An error occurred while generating report'
        });
...
exports.generateWeeklyPetrolReport = generateWeeklyPetrolReport;
const generateOilConsumptionReport = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    var _a;
    const { start_date, end_date } = req.query;
    const connection = yield db_1.default.getConnection();
    try {
        ...
        const [oilConfigRows] = yield connection.query(`SELECT config_value FROM app_config WHERE config_type = 'fuel' AND config_name = 'oil_codes'`);
        if (!oilConfigRows.length)
            throw new Error('Oil codes not configured');
        const oilCodes = oilConfigRows[0].config_value.split(',').map((c) => c.trim()).filter(Boolean);
        // 2. For each oil code, get total issued quantity and item details
        const oilData = [];
        for (const naccode of oilCodes) {
            // Get total issued quantity
            const [issueRows] = yield connection.query(`SELECT SUM(issue_quantity) as total_issued FROM issue_details WHERE nac_code = ? AND issue_date BETWEEN ? AND ? AND approval_status = 'APPROVED'`, [naccode, start_date, end_date]);
            const total_issued = Number((_a = issueRows[0]) === null || _a === void 0 ? void 0 : _a.total_issued) || 0;
            // Get item name, part number, unit
            const [stockRows] = yield connection.query(`SELECT item_name, part_numbers, unit FROM stock_details WHERE nac_code = ? LIMIT 1`, [naccode]);
        ...
        const [weekRows] = yield connection.query(`SELECT week_number FROM fuel_records f JOIN issue_details i ON f.issue_fk = i.id WHERE f.fuel_type = 'diesel' AND i.issue_date = ? LIMIT 1`, [start_date]);
        const weekNumber = weekRows.length ? weekRows[0].week_number : 0;
        // 4. Get today's date
        const today = new Date().toISOString().split('T')[0];
        // 5. Get level 3 authority name and designation
        const [authRows] = yield connection.query(`SELECT level_3_authority_name, level_3_authority_designation FROM authority_details WHERE authority_type = 'fuel' ORDER BY id DESC LIMIT 1`);
        ...
        yield connection.rollback();
        const errorMessage = error instanceof Error ? error.message : 'Unknown error occurred';
        (0, logger_1.logEvents)(`Error generating oil consumption report: ${errorMessage}`, 'fuelLog.log');
        res.status(500).json({
            error: 'Internal Server Error',
            message: errorMessage
        });
...
exports.generateOilConsumptionReport = generateOilConsumptionReport;
