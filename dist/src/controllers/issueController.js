...
exports.getDailyIssueReport = exports.deleteIssueItem = exports.updateIssueItem = exports.getPendingIssues = exports.rejectIssue = exports.approveIssue = exports.createIssue = void 0;
const db_1 = __importDefault(require("../config/db"));
const dateUtils_1 = require("../utils/dateUtils");
const logger_1 = require("../middlewares/logger");
const calculateIssueCost = (connection, nacCode, quantity) => __awaiter(void 0, void 0, void 0, function* () {
    // First check for received quantity and RRP total amount
    const [rrpResults] = yield connection.query(`SELECT 
    ...
    AND rrp.approval_status = 'APPROVED'`, [nacCode]);
    ...
    const [stockResults] = yield connection.query('SELECT open_quantity, open_amount FROM stock_details WHERE nac_code = ?', [nacCode]);
...
const createIssue = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    const { issueDate, items, issuedBy } = req.body;
    if (!issueDate || !items || !items.length || !issuedBy) {
        (0, logger_1.logEvents)(`Issue creation failed - Missing required fields by user: ${(issuedBy === null || issuedBy === void 0 ? void 0 : issuedBy.name) || 'Unknown'}`, "issueLog.log");
        res.status(400).json({
            error: 'Bad Request',
    ...
    try {
        ...
        const formattedIssueDate = (0, dateUtils_1.formatDateForDB)(issueDate);
        ...
        if (configRows.length === 0) {
            (0, logger_1.logEvents)(`Failed to create issue - Current FY configuration not found`, "issueLog.log");
            res.status(500).json({
        ...
        const [dayNumberResult] = yield connection.query(`SELECT 
        CASE 
      ...
      WHERE current_fy = ?`, [formattedIssueDate, currentFY]);
        ...
        for (const item of items) {
            ...
            const [stockResults] = yield connection.query('SELECT current_balance FROM stock_details WHERE nac_code = ?', [item.nacCode]);
            if (stockResults.length === 0) {
                (0, logger_1.logEvents)(`Issue creation failed - Item not found: ${item.nacCode} by user: ${issuedByName}`, "issueLog.log");
                throw new Error(`Item with NAC code ${item.nacCode} not found`);
            }
            const stockDetails = stockResults[0];
            const issueCost = yield calculateIssueCost(connection, item.nacCode, item.quantity);
            // Get all issues for this NAC code ordered by date
            const [allIssues] = yield connection.execute(`SELECT id, issue_date, issue_quantity, remaining_balance
         FROM issue_details
         WHERE nac_code = ?
         ORDER BY issue_date ASC`, [item.nacCode]);
            ...
            const [result] = yield connection.execute(`INSERT INTO issue_details (
          issue_date,
          nac_code,
          part_number,
          issue_quantity,
          ...
          approval_status
            ...
            yield connection.execute('UPDATE stock_details SET current_balance = current_balance - ? WHERE nac_code = ?', [item.quantity, item.nacCode]);
            (0, logger_1.logEvents)(`Item issued successfully - NAC: ${item.nacCode}, Quantity: ${item.quantity} by user: ${issuedByName}`, "issueLog.log");
        }
        yield connection.commit();
        (0, logger_1.logEvents)(`Issue created successfully for date: ${(0, dateUtils_1.formatDate)(issueDate)} by user: ${issuedByName}`, "issueLog.log");
        res.status(201).json({
            message: 'Issue created successfully',
            issueDate: (0, dateUtils_1.formatDate)(issueDate),
            issueSlipNumber,
            issueIds
        });
    }
        ...
        (0, logger_1.logEvents)(`Error in createIssue: ${errorMessage}`, "issueLog.log");
        res.status(500).json({
            error: 'Internal Server Error',
...
const approveIssue = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    try {
        ...
        const [issueCheck] = yield connection.execute(`SELECT id, approval_status 
       FROM issue_details 
       WHERE id IN (${issueIds.map(() => '?').join(',')})`, issueIds);
        if (issueCheck.length === 0) {
            (0, logger_1.logEvents)(`Failed to approve issues - No issues found with IDs: ${issueIds.join(', ')}`, "issueLog.log");
        ...
        if (alreadyApproved.length > 0) {
            (0, logger_1.logEvents)(`Failed to approve issues - Some issues are already approved: ${alreadyApproved.map(i => i.id).join(', ')}`, "issueLog.log");
            throw new Error(`Issues ${alreadyApproved.map(i => i.id).join(', ')} are already approved`);
        }
        // Get all issue details
        const [issueDetails] = yield connection.execute(`SELECT 
        i.id,
        i.nac_code,
        i.issue_quantity,
        i.issue_date,
      ...
      WHERE i.id IN (${issueIds.map(() => '?').join(',')})`, issueIds);
        ...
        const [stockBalances] = yield connection.query(`SELECT nac_code, current_balance FROM stock_details WHERE nac_code IN (${uniqueNacCodes.map(() => '?').join(',')})`, uniqueNacCodes);
        const balanceMap = new Map(stockBalances.map((row) => [row.nac_code, row.current_balance]));
        // Process each issue
        for (const issue of issueDetails) {
         ...
         FROM issue_details
         WHERE nac_code = ?
         ORDER BY issue_date ASC`, [issue.nac_code]);
            // Split issues into before and after current issue date
            ...
            yield connection.execute('UPDATE issue_details SET remaining_balance = ? WHERE id = ?', [adjustedBalance, issue.id]);
            // Update subsequent issues' remaining balances
            let runningBalance = adjustedBalance;
            for (const afterIssue of afterIssues) {
                if (afterIssue.id === issue.id)
                    continue; // Skip the current issue
                runningBalance -= afterIssue.issue_quantity;
                yield connection.execute('UPDATE issue_details SET remaining_balance = ? WHERE id = ?', [runningBalance, afterIssue.id]);
            }
        }
      ...
      WHERE id IN (${issueIds.map(() => '?').join(',')})`, [approvedBy, ...issueIds]);
        yield connection.commit();
        (0, logger_1.logEvents)(`Successfully approved issues with IDs: ${issueIds.join(', ')} by user: ${approvedBy}`, "issueLog.log");
        res.status(200).json({
        ...
        yield connection.rollback();
        const errorMessage = error instanceof Error ? error.message : 'Unknown error occurred';
        (0, logger_1.logEvents)(`Error approving issues: ${errorMessage} for IDs: ${issueIds.join(', ')}`, "issueLog.log");
        res.status(500).json({
            error: 'Internal Server Error',
...
exports.approveIssue = approveIssue;
const rejectIssue = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    try {
      ...
      WHERE i.id IN (${Array.isArray(itemIds) ? itemIds.map(() => '?').join(',') : '?'})`, Array.isArray(itemIds) ? itemIds : [itemIds]);
        if (issueDetails.length === 0) {
            (0, logger_1.logEvents)(`Failed to reject issues - No issues found with IDs: ${Array.isArray(itemIds) ? itemIds.join(', ') : itemIds}`, "issueLog.log");
            throw new Error('Issue records not found');
        }
        // Get the first issue's issued_by for notification
        const issuedBy = JSON.parse(issueDetails[0].issued_by);
        const [users] = yield connection.query('SELECT id FROM users WHERE username = ?', [issuedBy.staffId]);
        if (users.length > 0) {
            const userId = users[0].id;
            // Create a single notification with all issue details
            const issueDetailsText = issueDetails.map(issue => `Issue Slip: ${issue.issue_slip_number} (${(0, dateUtils_1.formatDate)(issue.issue_date)})`).join(', ');
        ...
        for (const issue of issueDetails) {
            yield connection.execute('UPDATE stock_details SET current_balance = current_balance + ? WHERE nac_code = ?', [issue.issue_quantity, issue.nac_code]);
        }
        // Delete all the issue records
        yield connection.execute(`DELETE FROM issue_details WHERE id IN (${Array.isArray(itemIds) ? itemIds.map(() => '?').join(',') : '?'})`, Array.isArray(itemIds) ? itemIds : [itemIds]);
        yield connection.commit();
        (0, logger_1.logEvents)(`Successfully rejected issues with IDs: ${Array.isArray(itemIds) ? itemIds.join(', ') : itemIds} by user: ${rejectedBy}`, "issueLog.log");
        res.status(200).json({
        ...
        yield connection.rollback();
        const errorMessage = error instanceof Error ? error.message : 'Unknown error occurred';
        (0, logger_1.logEvents)(`Error rejecting issues: ${errorMessage} for IDs: ${Array.isArray(itemIds) ? itemIds.join(', ') : itemIds}`, "issueLog.log");
        res.status(500).json({
            error: 'Internal Server Error',
...
exports.rejectIssue = rejectIssue;
const getPendingIssues = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    const connection = yield db_1.default.getConnection();
    try {
        const [issues] = yield connection.execute(`SELECT 
        i.id,
        i.nac_code,
        i.part_number,
        i.issue_quantity,
      ...
      ORDER BY i.issue_date DESC`);
        ...
        (0, logger_1.logEvents)(`Successfully retrieved ${formattedIssues.length} pending issues`, "issueLog.log");
        res.status(200).json({
        ...
        const errorMessage = error instanceof Error ? error.message : 'Unknown error occurred';
        (0, logger_1.logEvents)(`Error retrieving pending issues: ${errorMessage}`, "issueLog.log");
        res.status(500).json({
            error: 'Internal Server Error',
...
const updateIssueItem = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    const { id } = req.params;
    ...
    try {
        ...
        const [issueDetails] = yield connection.query(`SELECT 
        i.nac_code,
        i.issue_quantity,
        i.issue_slip_number,
        s.current_balance
      ...
      WHERE i.id = ?`, [id]);
        ...
        const issueCost = yield calculateIssueCost(connection, issue.nac_code, quantity);
        // Update the issue details
        yield connection.execute(`UPDATE issue_details 
      SET issue_quantity = ?,
          issue_cost = ?,
          updated_at = CURRENT_TIMESTAMP
      WHERE id = ?`, [quantity, issueCost, id]);
        ...
        if (quantityDifference !== 0) {
            ...
            yield connection.execute('UPDATE stock_details SET current_balance = current_balance - ? WHERE nac_code = ?', [quantityDifference, issue.nac_code]);
        }
        yield connection.commit();
        (0, logger_1.logEvents)(`Successfully updated issue item ID: ${id} with new quantity: ${quantity}`, "issueLog.log");
        res.status(200).json({
        ...
        yield connection.rollback();
        const errorMessage = error instanceof Error ? error.message : 'Unknown error occurred';
        (0, logger_1.logEvents)(`Error updating issue item: ${errorMessage} for ID: ${id}`, "issueLog.log");
        res.status(500).json({
            error: 'Internal Server Error',
...
exports.updateIssueItem = updateIssueItem;
const deleteIssueItem = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    const { id } = req.params;
    const connection = yield db_1.default.getConnection();
    try {
        yield connection.beginTransaction();
      ...
      WHERE i.id = ?`, [id]);
        ...
        yield connection.execute('DELETE FROM issue_details WHERE id = ?', [id]);
        // Add back the quantity to stock balance
        yield connection.execute('UPDATE stock_details SET current_balance = current_balance + ? WHERE nac_code = ?', [issue.issue_quantity, issue.nac_code]);
        yield connection.commit();
        (0, logger_1.logEvents)(`Successfully deleted issue item ID: ${id}`, "issueLog.log");
        res.status(200).json({
        ...
        yield connection.rollback();
        const errorMessage = error instanceof Error ? error.message : 'Unknown error occurred';
        (0, logger_1.logEvents)(`Error deleting issue item: ${errorMessage} for ID: ${id}`, "issueLog.log");
        res.status(500).json({
            error: 'Internal Server Error',
...
exports.deleteIssueItem = deleteIssueItem;
const getDailyIssueReport = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    `;
        ...
        // Parse the issued_by JSON string for each issue
        const formattedIssues = issues.map(issue => (Object.assign(Object.assign({}, issue), { issued_by: JSON.parse(issue.issued_by) })));
        (0, logger_1.logEvents)(`Successfully generated daily issue report from ${fromDate} to ${toDate}${equipmentNumber ? ` for equipment ${equipmentNumber}` : ''}`, "issueLog.log");
        res.status(200).json({
        ...
        const errorMessage = error instanceof Error ? error.message : 'Unknown error occurred';
        (0, logger_1.logEvents)(`Error generating daily issue report: ${errorMessage}`, "issueLog.log");
        res.status(500).json({
            error: 'Internal Server Error',
